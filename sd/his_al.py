...
def correlation_pair(df):
    return df[['MNST_Return', 'MSFT_Return', 'AXP_Return', 'SPY_Return', 'IXIC_Return']].corr()

def avg_corr(df):
    df['Mean_return'] = df[['MNST_Return', 'MSFT_Return', 'AXP_Return']].mean(axis=1)
...
# Historical amalysis for 3 benchmarks tickers and 2 market indices
def main():
    conn = duckdb.connect("test.ddb")
    try:
        df = conn.sql(
            "SELECT Date, MNST_Close, MSFT_Close, AXP_Close, SPY_Close, IXIC_Close "
            "FROM yahoo "
            "WHERE Date < '2026-3-2'"
        ).fetchdf()
